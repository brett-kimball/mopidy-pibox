...
**Note:** In order to understand if people are using Pibox, `GoatCounter <https://www.goatcounter.com>`_ is used to track basic usage. It does not store any personal data. If you do not want analytics to be collected, you can disable it by setting the ``disable_analytics`` config option to ``true`` in your Mopidy config file.
...
- ``pibox/disable_analytics``: Stops `GoatCounter <https://www.goatcounter.com>`_ analytics from being included in the Pibox web app. Defaults to ``false``.
